prediction_futur_rf <- function(model, input_file, predicteurs_path, shapefile_path, output_folder)
  {
  model <- model$model
  ...
  for (scenario in unique_scenarios) {
    ...
    for (file_path in scenario_files) {
      ...
      # Prédire avec la fonction predict (dismo package)
      pred_rf <- dismo::predict(spatraster1, model,
                                fun = function(model, ...)
                                  dismo::predict(model, ...)$predictions[,"1"])
      ...
      terra::writeRaster(pred_rf, filename = output_file, format = "GTiff", overwrite = TRUE)
    }
  }
