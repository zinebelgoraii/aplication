prediction_futur_xgboost <- function(model, input_file, predicteurs_path, shapefile_path, output_folder)
  {
  model <- model$model
  ...
  for (scenario in unique_scenarios) {
    ...
    for (file_path in scenario_files) {
      ...
      predictors_matrix <- raster::as.matrix(spatraster1)
      
      # Faire des prédictions avec votre modèle XGBoost
      predictions <- raster::predict(model, newdata = predictors_matrix, ext = ext_shapefile)
      # Create an empty raster with the same dimensions as the input raster
      predictions_raster <- raster::raster(ext_shapefile, nrows = nrow(spatraster1), ncols = ncol(spatraster1))
      ...
      raster::projection(predictions_raster) <- raster::projection(spatraster1)
